...
**Implemented baseline:** tag `v0.1.0` (commit `3c8ab8d`) is published at <https://github.com/nayvok/Aptiloop/releases/tag/v0.1.0> with the npm launcher tarball, five platform runtime archives, `SHA256SUMS`, `DIGEST`, CycloneDX and SPDX SBOMs, `NOTICE`, and `THIRD_PARTY_NOTICES.md`. The pinned release workflow built every asset (its first attempt exposed a missing Playwright Chromium install in the validate job, now fixed on `main`), the handoff digests match `SHA256SUMS`, and the GitHub asset digests match the same sums.

**Implemented baseline:** a disposable Windows smoke installed the released `aptiloop-0.1.0.tgz` into a clean prefix, ran the launcher from a directory outside any source checkout, and observed the full public path: the launcher downloaded the `v0.1.0` win32-x64 runtime from the GitHub release, verified the SHA-256 sums and the GitHub asset digest, extracted and hash-verified the runtime manifest (17,684 files), switched the installed pointer atomically, and reported `--version` → `0.1.0`. This verifies public tagged-release downloading for the installed CLI on Windows; npm publication, macOS/Linux installed runs, and post-switch rollback remain **UNVERIFIED**. A tagged runtime release is not Core Alpha release acceptance: the owner-approved M12 acceptance gates above still apply.

The earlier release-preparation smoke (hash-verified runtime, tarball install against a locally prepared runtime) is superseded by the public path above; the hosted CI history for these trees is green (runs #22–#26), including the first fully green hosted CI run for the repository.
