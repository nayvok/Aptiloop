...
Migration `0011_course_pack_lifecycle` stores immutable validated manifest identity/canonical JSON, source-byte hash, validation report, provenance, append-only install/open-as-draft/uninstall lifecycle events, and bounded invalid-pack quarantine diagnostics. The public delete contract is Course-scoped and has no restore path: it archives every revision, clears learner pointers, and excludes the Course from library projections. Imported Course/revision/source/capsule rows and completed learner evidence remain hash-bound immutable internal records. The historical `uninstall` event value is the compatible storage tombstone and is not exposed as a user-facing archive state.
...
| Learner Course state                        | Server-owned projection                      | Active target operations only              | Course-level delete only when no active scoped session remains |
