...
Каждый `/api/*` request требует `X-Aptiloop-Client: web` (кроме test environment). Mutations требуют loopback `Origin` на порту из `WEB_ORIGIN` (`127.0.0.1`, `localhost` или `[::1]`), и `Content-Type: application/json`. Bodies проверяются strict Zod schemas.
