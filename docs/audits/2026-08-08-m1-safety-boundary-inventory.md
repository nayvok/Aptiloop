...
- every `/api/*` response is `Cache-Control: no-store`; mutation request-shape controls remain exact Origin, `X-Aptiloop-Client=web`, and JSON media type. These controls are not authentication.
