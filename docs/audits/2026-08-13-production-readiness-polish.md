...
It is working-tree evidence, not a claim about commit `b542b32` or any future commit. At this cutoff, release authorization, a project license/notices/trademark set, hosted CI, and owner sign-off remained separate gates. ADR 0010 later superseded the no-license state by applying Apache-2.0 to covered first-party repository materials; that later decision does not alter this audit's technical evidence or close its other artifact gates. Aptiloop intentionally ships no Course; approval applies only if a future first-party or sample Course is proposed.
...
- `docker compose config --format json` resolved two loopback-published services with non-root image users, read-only root filesystems, dropped capabilities, `no-new-privileges`, bounded temporary filesystems, named private-data volumes, and no exercise-fixture bind mount. A clean lockfile production-install probe retained the Hono/Pi/OpenCode runtime closure while excluding representative development packages (`vitest`, `tsx`, `tsup`, TypeScript, Next, and Playwright). At the audit cutoff Docker Desktop's Linux daemon was not running, so that cutoff did not claim a fresh image build, filesystem inspection, or container health result. A later 2026-08-13 disposable live probe built both production images, observed healthy loopback-only web and readiness responses, verified the hardened runtime configuration, stopped cleanly, removed every disposable container/network/volume/image, and left existing data volumes untouched.
...
- At the audit cutoff, an approved project license plus notices/content/trademark review. ADR 0010 later closed only the project-license choice and application.
