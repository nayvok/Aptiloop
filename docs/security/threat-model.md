...
- **Observed test:** malformed JSON/encoding/BOM/duplicates, byte/depth/item/string/parse budgets, forbidden paths/authority/secrets/URLs, graph/locale/reference/requirements errors, hash mismatch/collision, rollback/cleanup, canonical Kit/import parity, idempotent re-import, active-session deletion rejection, Course-level library removal, and immutable evidence retention.
