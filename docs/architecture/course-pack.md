...
**Implemented baseline.** Migration `0011_course_pack_lifecycle` stores immutable canonical manifests/localizations/knowledge nodes plus append-only lifecycle events and bounded quarantine records; it does not store pre-commit staging handles or files. Same-action re-import is idempotent; cross-action, key/hash, occupied-branch, and existing-Course primary-locale conflicts fail closed. The user-facing delete operation is Course-scoped: one destructive confirmation hides the Course and all revisions from Courses, clears current learner pointers, disables direct canonical export for the deleted Course, and offers no restore action. It is rejected while any active learning session pins the Course. Published revisions, completed sessions, manifests, and evidence remain immutable internal records; the existing append-only `uninstall` lifecycle value is retained only as the storage tombstone for compatibility with migration `0011`. The Courses UI exposes one accessible file-selection control, validation report/Preview/provenance/requirements/hash, explicit Install/Open-as-draft, revision-scoped canonical export for visible Courses, and confirmed permanent Course deletion.

**Implemented baseline.** A validated Pack whose `courseKey` matches an installed Course, whose revision number is strictly greater, and whose parent revision resolves in that Course is staged as an upgrade rather than a second ordinary install. Preview classifies stable activities as carried, requiring revalidation, or removed and exposes bounded personal-adaptation conflicts. The user chooses exactly one explicit consequence: `safe-update` (the default) or `side-by-side`. Safe-update is blocked while any old-revision session is active; otherwise it publishes the incoming immutable upstream revision, keeps old facts and revisions unchanged, replays only facts whose activity and prerequisite contracts still match, and retains resolved personal divergence as archived rebased lineage. New learning activates the incoming published revision. Side-by-side derives a collision-checked Course identity and installs without learner history.
...
**Implemented baseline.** An ordinary consequence requires a strict POST carrying a unique operation ID, the selected `install | open-as-draft` action, and the exact Preview content hash. A detected upgrade instead uses a dedicated strict POST carrying the operation ID, exact hash, `safe-update | side-by-side`, optional side-by-side suffix, and an exact resolution for every reported adaptation conflict. The server claims the staged validation before asynchronous file/database work, re-reads and revalidates the staged bytes and both hashes, then performs one transaction. A consumed operation can be reconciled idempotently only when its validation, mode, content hash, suffix, and normalized resolutions match; any mismatch fails closed.
...
- Resolve identity conflicts and show the user the exact target Course/revision. A strictly newer child revision of an installed Course exposes the safe-update/side-by-side upgrade preview.
- Persist Course, revision, activities, sources, capsules, and provenance in one transaction.
- Apply only the explicitly selected consequence: ordinary Install publishes and activates the imported immutable revision; Open-as-draft archives the canonical imported source and creates a separate mutable personal Draft. For an upgrade, safe-update preserves immutable history and replays only contract-compatible facts, while side-by-side creates a distinct Course with no carried history.
- On failure, roll back the transaction and retain only a bounded diagnostic; never partially merge.
- Re-import and upgrade replay are deterministic and idempotent only for the exact operation payload. Changed action, mode, content hash, suffix, or adaptation resolution conflicts.
