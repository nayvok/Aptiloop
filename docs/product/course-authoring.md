...
The current Aptiloop application has an Adaptive Studio workflow backed by versioned curriculum tables. It can create or clone a draft revision, edit/reorder week/day/unit records through typed forms, validate, Preview, review changes, and explicitly publish an immutable revision with a content hash. The optional Course Designer persists the owner-approved Initial Brief → Discovery → Diagnostic when level or prerequisite knowledge is uncertain (with an explicit decline/skip assumption) → Learning Design → Course Proposal sequence, requires an explicit assumption when Diagnostic is skipped, reads only the selected Draft and explicitly approved sources, returns strict stable-ID changes with immutable provider/model/prompt attribution, and requires separate Apply, Preview, Changes, and Publish decisions.
...
| `DIAGNOSTIC`          | Run bounded questions or practical tasks when level or prerequisite knowledge is uncertain. A decline or skip remains available but must be recorded as an explicit Learning Design/Proposal assumption; diagnostic output is evidence for the proposal, not mastery.                                                                             |
| `LEARNING_DESIGN`     | Persist the required target capability → observable evidence → practice → feedback → instruction/review chain before proposal generation. Use attempt-before-answer, changed-condition transfer, explicit mastery evidence, and honest runtime-unavailable degradation; keep interview readiness and independent engineering capability separate. |
...
Aptiloop provides a self-contained, version-matched authoring skill for a capable external model without granting that model Aptiloop tools or local authority. The lightweight Initial Brief seeds the owner-approved conversational lifecycle—Discovery, Diagnostic when level or prerequisite knowledge is uncertain (with an explicit decline/skip assumption), mandatory Learning Design, Course Proposal, explicit User Review, Compilation, deterministic Validation/Repair, learner-safe Preview, and explicit local commit. Interactive mode is the default; only an explicitly selected non-interactive function call may compile directly from the brief, and it must preserve unresolved facts and the same Learning Design contract rather than pretending that review occurred.
...
- Authoring Kit, format major/minor, validator, and `skillContentVersion` identities; import Preview shows a non-blocking warning when the Pack's authoring-skill content version differs from the current build;
...
1. From Course creation, choose **Use an external model**, complete the Initial Brief, and download the commit-pinned, content-versioned skill. Aptiloop sends nothing automatically.
2. Give the chosen model only the skill and context the user intends to disclose. During Discovery, use Diagnostic when level or prerequisite knowledge is uncertain; if it is declined or skipped, record an explicit assumption.
3. Complete Learning Design before the Course Proposal: map capability to evidence, practice, feedback, and instruction/review; use attempt-before-answer and changed-condition transfer; state objective trade-offs, mastery evidence, unresolved facts, and runtime-unavailable degradation explicitly.
4. Review and explicitly approve the exact Course Proposal. A material revision requires approval again; a Proposal without completed Learning Design is invalid for compilation.
5. Receive one UTF-8 hashless Authoring Draft JSON document, then open `/courses/import` and select that exact file. No Pack file input exists on `/courses/new`.
6. Aptiloop deterministically finalizes and validates locally. Preserve each failed draft and repair from exact diagnostics; do not hand-edit requirements or hashes.
7. Review provenance, derived requirements, final canonical identity, finalization provenance, learner-safe Preview, and any non-blocking authoring-skill version mismatch warning.
8. Explicitly choose **Install immutable revision** or **Open as local Draft**. Installation records the immutable source revision; Open as Draft preserves that source manifest and creates a separate editable personal lineage. Neither action publishes the Draft.
