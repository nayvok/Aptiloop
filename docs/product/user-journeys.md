...
| 3. Course Pack import                 | **Implemented baseline**                                  | Bounded validation, temporary URL-restored intake, learner-safe Preview, atomic Install/Open-as-draft, exact lineage/revision export, and fail-closed permanent Course deletion exist. Deletion removes the Course from user-facing library projections while retaining immutable completed evidence internally. Process restart requires file reselection. |
