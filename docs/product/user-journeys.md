...
8. Before a later provider request, the transmission review names role, provider/model, destination, selected Draft fields/source records, payload categories, exclusions, and retention. Course Designer may then ask bounded questions and, when level or prerequisite knowledge is uncertain, offer a Diagnostic; a decline or skip is recorded as an explicit assumption. It completes the mandatory Learning Design and returns a typed proposal with finite structure, sequencing, sources/capsules, activity/evidence types, trusted runtime/check references, estimates, assumptions, and provider/model provenance.
...
3. The external model treats the brief as seed context, performs Discovery, asks only material missing questions, uses a bounded Diagnostic when level or prerequisite knowledge is uncertain (recording an explicit decline/skip assumption), completes the mandatory Learning Design, and then presents a finite Course proposal. The normal human workflow cannot compile until the user explicitly approves that proposal; an explicitly requested non-interactive run must disclose its assumptions.
...
12. For a new Course, the user explicitly chooses **Install immutable revision** or **Open as local draft**, reviews the exact finalized identity/hash/destination consequence, and confirms one atomic commit. Open as draft preserves the imported manifest as an immutable archived source revision and creates a separate personal Draft/adaptation lineage.
13. For a strictly newer child revision of an installed Course, Preview instead shows the revision transition, activities whose contracts are carried or require revalidation, removed activities, personal-adaptation conflicts, and the collision-checked side-by-side identity. The default explicit choice is **Safe update**; **Install side by side** is the history-free alternative.
...
- Permanent Course deletion hides every revision without a restore action, preserves immutable internal history, and fails closed while an active learning session pins the Course.
...
- Safe-update fails closed while an old-revision session is active. After that session is completed or abandoned, safe-update activates the incoming published revision for new learning, keeps old revisions/sessions/facts immutable, replays only contract-compatible facts, and preserves resolved personal divergence as archived rebased lineage.
- Side-by-side creates a distinct Course and carries no learner history.
...
| 3. Course Pack import                 | **Implemented baseline**                                  | Bounded validation, temporary URL-restored intake, learner-safe Preview, atomic Install/Open-as-draft, and explicit safe-update/side-by-side handling exist. Safe-update blocks active old-revision sessions, preserves immutable history, and replays only contract-compatible facts; side-by-side carries no history. Exact lineage/revision export and fail-closed permanent Course deletion remain available. Process restart requires file reselection. |
...
The baseline intentionally supplies no bundled Course. A fresh user creates a personal Course or imports a trusted Course Pack; this is normal product behavior, not a release blocker. The Apache-2.0 public source grant remains distinct from external data sharing and a tagged Core Alpha release. User/imported Courses and any future first-party/sample Course retain their applicable privacy, legal, content, artifact, and owner gates.
