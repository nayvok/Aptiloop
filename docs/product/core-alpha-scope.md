...
The runnable code and npm workspaces now carry the Aptiloop / `@aptiloop/*` identity. M1–M11 are implemented around the original vertical slice, including safety containment; strict Course/revision/section/lesson/finite-Activity/source/capsule/adaptation/session-context/evidence/review contracts; bounded declarative Course Pack V1 validation and lifecycle; replay-complete deterministic Learning Kernel facts/projections; trusted local Execution Fabric contracts; the constrained Provider Hub; complete `en-US`/`ru-RU` UI catalogs; the closed Activity Frame registry; Adaptive Studio; and the Course/session cutover. M12 technical preflight, hosted CI, and authenticated real-provider smoke are also **Implemented baseline** evidence, not release authorization.

The web baseline implements the Calm Workshop shell with Home, Courses, Review, Skills, and Settings; a stable collapsible desktop rail; mobile bottom navigation; entity-aware breadcrumbs; responsive light/dark layouts; URL-restored route state where specified; visible focus; semantic landmarks/headings; reduced-motion behavior; and localized recovery states. This is observed implementation evidence, not complete WCAG 2.2 conformance or owner acceptance.

Pending-disclosure recovery for Course Designer and Interview has integration and component-remount evidence only. Course Designer recovery-preview freshness is narrower than Interview recovery and is documented in the Provider Hub boundary. No fresh authenticated supported-provider smoke has been observed for commit `b542b32`; the real-provider release gate remains open.
...
All 50 gates below are release-blocking and are fixed before Core Alpha release acceptance. M12 must attach observed evidence to every row. A missing, failed, stale, inferred, or mock-only result does not close a gate. A proposed deferral changes the approved release scope and therefore requires a new explicit owner decision; it is not an implicit waiver.
...
## Dated M12 gate evidence (2026-08-10)

**Implemented baseline**

This section preserves historical evidence for the tree exercised on 2026-08-10. It does not certify later changes, close an open gate, or authorize release. `npm ci`, `npm run verify`, and `npm run test:e2e` passed for that tree; E2E was 4/4. An authenticated OpenCode Zen `deepseek-v4-flash-free` typed-role smoke completed through constrained Pi with exact disclosure consumption, persisted minimal provenance, explicit no-fallback provider/model resolution, and observed cancellation using synthetic text in a disposable database. Direct local-process and loopback-published Compose topology, quiesced backup/restore/rollback, a real Python Environment Pack, SBOM/dependency policy, and candidate-container content inspection were also exercised for that dated tree.
Hosted GitHub Actions [CI run #4](https://github.com/nayvok/Aptiloop/actions/runs/31418920332) then passed all three jobs for commit `3a5391b`: Supply chain, Fast quality gates, and End-to-end gate. The two preceding hosted runs exposed and led to fixes for a Linux-only reviewer workspace-path disclosure and migration-heavy database test timeouts rather than being waived or rerun unchanged.

The following dated disposition attaches the observed M12 evidence to every fixed matrix row. “Passed locally” is implementation evidence for the 2026-08-10 tree, not release authorization or evidence for later changes. A row remains open whenever any required observation or approval is unavailable.

|   # | M12 disposition | Observed evidence                                                                                                                                                                                                                |
| --: | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1 | Passed locally  | Clean direct and Compose launches returned web `200` and orchestrator readiness `200`; browser smoke observed no page errors.                                                                                                    |
|   2 | Passed locally  | Complete `en-US` catalog, localized component tests, and English browser journeys passed.                                                                                                                                        |
|   3 | Passed locally  | Complete `ru-RU` catalog, localized component tests, and Russian browser journeys passed.                                                                                                                                        |
|   4 | Passed locally  | Required screens were exercised at 320 px and representative mobile viewport sizes without horizontal overflow.                                                                                                                  |
|   5 | Passed locally  | Light/dark hydration E2E passed; light, dark, and system selectors were exercised in browser QA.                                                                                                                                 |
|   6 | Passed hosted   | GitHub Actions CI run #4 passed Supply chain, Fast quality gates, and End-to-end gate on Node 24 for commit `3a5391b`.                                                                                                           |
|   7 | Passed locally  | `npm run test:e2e` passed all 4 supported scenarios.                                                                                                                                                                             |
|   8 | Passed locally  | Additive upgrades, active-data inventories, non-overwriting backups, current admission, disposable restore, integrity, foreign-key, migration-ledger, and rollback checks passed.                                                |
|   9 | Passed locally  | Activity Graph/kernel suites and the restart-safe learning E2E passed.                                                                                                                                                           |
|  10 | Passed locally  | Attempt/evidence replay tests preserve the immutable unaided first response.                                                                                                                                                     |
|  11 | Passed locally  | Bounded Tutor/provider-policy suites and the manual learning fallback passed without model-owned progression.                                                                                                                    |
|  12 | Passed locally  | Quiz scoring, retry, protected-answer, evidence, and Activity Frame tests passed.                                                                                                                                                |
|  13 | Passed locally  | Code-reading prediction, explanation, protected feedback, and evidence tests passed.                                                                                                                                             |
|  14 | Passed locally  | Recall first-attempt, delayed-answer, revision, and deterministic transition tests passed.                                                                                                                                       |
|  15 | Passed locally  | Learner explanation and deterministic/manual completion tests passed.                                                                                                                                                            |
|  16 | Passed locally  | Versioned summary derivation and idempotent persistence tests passed.                                                                                                                                                            |
|  17 | Passed locally  | Checkpoint criteria, failure/lock, and kernel transition tests passed.                                                                                                                                                           |
|  18 | Open            | Due reason, prior-evidence provenance, and deterministic scheduling projections pass. No typed server-verified executor yet completes or reschedules a due item; the ordinary source-session shortcut is intentionally withheld. |
|  19 | Passed locally  | Isolated workspace, bounded document/save/diff/resume suites and exercise E2E passed.                                                                                                                                            |
|  20 | Passed locally  | Shell-free app-owned editor plan and copy/manual fallback tests passed.                                                                                                                                                          |
|  21 | Passed locally  | Trusted environment/check registry permitted known IDs and rejected forged IDs.                                                                                                                                                  |
|  22 | Passed locally  | Bounded structured output, truncation/status, and redaction tests passed.                                                                                                                                                        |
|  23 | Passed locally  | Complete workspace snapshot/diff and overflow-negative suites passed.                                                                                                                                                            |
|  24 | Passed locally  | Review request provenance, exact context, freshness, provider/model, and evidence fingerprint tests passed.                                                                                                                      |
|  25 | Passed locally  | Reviewer tool denial and before/after workspace integrity tests passed.                                                                                                                                                          |
|  26 | Passed locally  | Restart-safe correction E2E kept requested changes open until edit, fresh check, and fresh review.                                                                                                                               |
|  27 | Passed locally  | Stale check/review fingerprint rejection tests passed after edits.                                                                                                                                                               |
|  28 | Passed locally  | Versioned Node environment happy/failure/limit/cleanup contract suites passed.                                                                                                                                                   |
|  29 | Passed locally  | Versioned Python environment suites passed, plus a real disposable-container Python happy/failure smoke.                                                                                                                         |
|  30 | Passed locally  | Node/Python registry and operation-isolation tests passed without identifier or result collisions.                                                                                                                               |
|  31 | Passed locally  | Missing-runtime UI tests preserve work and expose exact non-secret recovery guidance.                                                                                                                                            |
|  32 | Passed locally  | Spawn, timeout, cancellation, corruption, cap, and process-tree cleanup suites passed.                                                                                                                                           |
|  33 | Passed locally  | Typed append-only evidence, operation provenance, linkage, clock, and idempotency suites passed.                                                                                                                                 |
|  34 | Passed locally  | Deterministic mastery/replay suites passed.                                                                                                                                                                                      |
|  35 | Passed locally  | Skills provenance/component tests and Skills E2E passed without treating interview observations as mastery.                                                                                                                      |
|  36 | Passed locally  | Replayable mistake projection tests and mistakes E2E passed.                                                                                                                                                                     |
|  37 | Passed locally  | Deterministic due-item/date scheduling tests passed.                                                                                                                                                                             |
|  38 | Passed locally  | Fact-derived card candidate, approval/export tests and cards E2E passed.                                                                                                                                                         |
|  39 | Open            | The 2026-08-10 smoke covered an earlier tree. No fresh authenticated supported-provider typed-role smoke has been observed for `b542b32`; integration/remount recovery evidence is not a substitute.                             |
|  40 | Passed locally  | Provider/auth/model/tool failure suites remained explicit and never selected Mock.                                                                                                                                               |
|  41 | Passed locally  | Learning activities with typed executors and manual authoring paths passed with AI Off; due-Review execution remains open under row 18.                                                                                          |
|  42 | Passed locally  | Localized provider failure UI tests preserved input and exposed only safe retry/settings/AI-Off choices.                                                                                                                         |
|  43 | Passed locally  | Missing/invalid credential and ownership-source diagnostics passed without secret disclosure.                                                                                                                                    |
|  44 | Passed locally  | Settings diagnostics covered Node, Python, editor, Core, and storage recovery actions.                                                                                                                                           |
|  45 | Passed locally  | Manual AI-Off draft → Preview → Changes → Publish → immutable-history/clone flow passed.                                                                                                                                         |
|  46 | Passed locally  | Course Pack validation, temporary intake recovery, learner-safe Preview, atomic install/open-as-draft, immutable source/personal lineage, canonical export, active-session uninstall rejection, and rollback suites passed.      |
|  47 | Passed locally  | Database guards, API rejection, Studio read-only state, clone, and negative mutation tests passed.                                                                                                                               |
|  48 | Passed locally  | Personal-branch publish and explicit upstream integration preserved source revision/evidence hashes.                                                                                                                             |
...
M12 is therefore **not accepted** and no Core Alpha release candidate was published. Technical preflight evidence is recorded, but row 18 remains open pending a typed server-verified due-Review executor and row 39 remains open pending a fresh authenticated supported-provider smoke on `b542b32`. `nextActionHref` remains `null`; Aptiloop does not fabricate a `/session` CTA from source-session identity. Release acceptance also remains blocked by professional legal review, approved license text, third-party notices, content/fixture terms, trademark policy, artifact authorization, and owner sign-off. No license grant or public-distribution approval is implied.
