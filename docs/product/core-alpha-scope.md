...
The runnable code remains Dev Learning Harness in package identity while M1–M10 are implemented around its vertical slice. The baseline includes safety containment; strict Course/revision/section/lesson/finite-Activity/source/capsule/adaptation/session-context/evidence/review contracts; bounded declarative Course Pack V1 validation and lifecycle; replay-complete deterministic Learning Kernel facts/projections; trusted local Execution Fabric contracts; the constrained Provider Hub; complete `en-US`/`ru-RU` UI catalogs and primary navigation; the closed Activity Frame renderer registry; Adaptive Studio manual authoring with a separate Personal Adaptation Branch; and the optional persisted Course Designer for typed, attributable, explicitly applied Draft proposals. Legacy tables/routes and bundled development curriculum remain visible compatibility surfaces.
