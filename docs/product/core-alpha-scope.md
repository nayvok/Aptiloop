...
The runnable code remains Dev Learning Harness in package identity while M1–M11 are implemented around its vertical slice. The baseline includes safety containment; strict Course/revision/section/lesson/finite-Activity/source/capsule/adaptation/session-context/evidence/review contracts; bounded declarative Course Pack V1 validation and lifecycle; replay-complete deterministic Learning Kernel facts/projections; trusted local Execution Fabric contracts; the constrained Provider Hub; complete `en-US`/`ru-RU` UI catalogs and Calm Workshop shell; the closed Activity Frame registry; manual/personal Adaptive Studio; the persisted guided Course Designer; and explicit per-Course learner selection, active revision, and current-session ownership.
...
The baseline is not the complete Core Alpha product merely because M1–M11 exist. Production Course content and approved Source Snapshots/Knowledge Capsules, release hardening, licensing/legal approval, independently isolated untrusted execution, and public distribution remain **Approved Core Alpha target** or **Future** work as classified elsewhere. M11 retains compatibility tables and the 526 unresolved migration quarantine rows as preserved history, not valid target facts; destructive removal remains separately gated.
...
M12 technical preflight was exercised locally on 2026-08-10. `npm ci`, `npm run verify`, and `npm run test:e2e` passed; E2E was 4/4. `npm run verify` and the 4/4 E2E gate were rerun after the reviewed provider catalog and connection-management cutover. An authenticated OpenCode Zen `deepseek-v4-flash-free` typed-role smoke completed through constrained Pi with exact disclosure consumption, persisted minimal provenance, explicit no-fallback provider/model resolution, and observed cancellation using synthetic text in a disposable database. Both the direct local-process topology and the loopback-published Compose topology returned healthy web and orchestrator responses. A quiesced Compose database was copied without overwrite, verified byte-identical, restored into a newly named volume, opened with `integrity_check=ok`, zero foreign-key violations, and all 19 migrations, then rolled back to the original named volume. A real Python Environment Pack ran in a disposable Debian container. The generated CycloneDX SBOM contained 482 components and 483 dependency records. `npm run audit:policy` passed with zero Critical, High, Moderate, Low, or Info findings in both production and full installed-tree audits. Candidate container inspection found no `.env`, key, SQLite, approved-backup, attempt, or private runtime payload; the web image contained no shipped exercise fixture tree, while the orchestrator image contained the 38 intended synthetic exercise fixture files.
...
The following disposition attaches the observed M12 evidence to every fixed matrix row. “Passed locally” is implementation evidence, not release authorization. A row remains open whenever any required observation or approval is unavailable.
...
|  18 | Passed locally  | Review reason, prior-evidence, completion, scheduling tests, and Review E2E passed.                                                                                                   |
...
|  39 | Passed locally  | Authenticated OpenCode Zen `deepseek-v4-flash-free` completed a constrained Pi typed-role smoke with one-time disclosure, minimal provenance, cancellation, and no fallback.          |
|  40 | Passed locally  | Provider/auth/model/tool failure suites remained explicit and never selected Mock.                                                                                                    |
|  41 | Passed locally  | Required learning and manual authoring paths passed with AI Off.                                                                                                                      |
...
|  46 | Passed locally  | Course Pack validation, Preview, explicit install/open-as-draft, canonical hash, export, and rollback suites passed.                                                                  |
...
M12 is therefore **not accepted** and no Core Alpha release candidate was published. Technical gate evidence is complete, but release acceptance remains blocked by professional legal review, approved license text, third-party notices, content/fixture terms, trademark policy, artifact authorization, and owner sign-off. No license grant or public-distribution approval is implied.
