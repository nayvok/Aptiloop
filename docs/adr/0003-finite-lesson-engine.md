...
- The app owns legal transitions (`blocked`, `ready`, `in_progress`, `complete`, and terminal failure/cancellation where applicable). The browser and models request actions; they do not assign authoritative status.
...
- A session pins immutable authored content. Resume reconstructs the next legal action from persisted snapshot, activity progress, and evidence rather than provider memory.
