...
After Discovery and Diagnostic, complete a Learning Design with target capability, observable evidence, practice, feedback, and instruction/review before presenting a Course Proposal. The Learning Design is not complete unless every skill or procedure names an observable mastery evidence type. Include attempt-before-answer, a changed-condition transfer check, the software decision-practice loop, and separate interview-readiness and engineering-capability time trade-offs.
...
The calling function explicitly selected non-interactive mode. Do not pretend that a conversational review occurred. Treat the supplied Initial Brief as the automation authorization to compile one hashless authoring draft directly, but compilation is invalid unless a completed Learning Design follows Discovery and Diagnostic and the Course Proposal records it. Do not ask questions; preserve every unresolved fact explicitly, never silently invent placeholders, provenance, source facts, or approvals, and expect deterministic validation to block incomplete claims. If a diagnostic is skipped, record that skip as an explicit Proposal assumption. This mode does not authorize validation bypass, installation, Open as Draft, or publication.`;
