...
Direct unauthenticated operation is fail-closed to exact loopback hosts. Only the explicit Compose mode may bind the container service to `0.0.0.0`, with host publication still fixed to `127.0.0.1`. All API responses are `Cache-Control: no-store`; mutation Origin/client/media-type checks remain request-shape controls, not identity. After those checks, JSON mutations are admitted under a 1 MiB pre-parse byte limit, the process admits at most 16 concurrent API requests and 4 concurrent Tutor SSE responses, and excess capacity receives a bounded `413`, or a `429` response with `Retry-After: 1`. Shutdown closes admission to new API work and drains admitted handlers and streams before closing SQLite; a drain timeout fails closed without closing the database. Course Pack validation retains its separate 1 MiB raw-byte validator.
...
The repository's current SQLite migration contract is the exact `0000`–`0021` ledger. `0019_provider_connection_retirement` adds the tombstone used to remove active provider configuration while preserving historical evidence. `0020_adaptation_branch_lifecycle` retains archived branch/fact identity while enforcing one learner-active personal branch, exact base/head ownership, immutable revision-compatible session-to-branch pinning, and matching Kernel write scope per Course. Archived authoring branches cannot receive learner sessions, and Open-as-draft cannot rotate learner scope. A newer revision cannot rotate Course/learner state while an active session is pinned to the current revision. `0021_learning_kernel_fact_schema_v2` rebuilds the Learning Kernel fact table so its versioned envelope may carry v1 legacy facts and v2 migration-lineage facts, and persists the Learning Design stage in Course Designer workflows without rewriting any existing row. Ordinary startup admits that current contract without rewriting history; advancing an admitted predecessor with valuable data requires the explicit backup-bound operation above.
...
| SEC-REVIEW-001    | High   | Implemented baseline       | Reviewer receives only a bounded evidence capsule and has no write, patch, filesystem, process, or network tool; its advisory verdict cannot decide progression.                                              |
...
| SEC-EVIDENCE-001  | High   | Implemented baseline       | Reviewer output is non-authoritative advice. Only a server-validated immutable review receipt bound to exact fresh trusted-check evidence can satisfy participation; it cannot emit correct mastery evidence. |
...
| SEC-HTTP-001      | Low    | Implemented baseline       | JSON mutations are byte-bounded before parsing and API/Tutor-stream concurrency is process-bounded with explicit `413`/`429` responses.                                                                       |
