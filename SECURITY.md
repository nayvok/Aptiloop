...
The repository's current SQLite migration contract is the exact `0000`–`0020` ledger. `0019_provider_connection_retirement` adds the tombstone used to remove active provider configuration while preserving historical evidence. `0020_adaptation_branch_lifecycle` retains archived branch/fact identity while enforcing one learner-active personal branch, exact base/head ownership, immutable revision-compatible session-to-branch pinning, and matching Kernel write scope per Course. Archived authoring branches cannot receive learner sessions, and Open-as-draft cannot rotate learner scope. A newer revision cannot rotate Course/learner state while an active session is pinned to the current revision. Ordinary startup admits that current contract without rewriting history; advancing an admitted predecessor with valuable data requires the explicit backup-bound operation above.
