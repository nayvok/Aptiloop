...
The runnable application remains the **Dev Learning Harness** in package identity while the UI now carries the Aptiloop identity. M1 safety containment and M2 Course foundations remain enforced; M3 provides the declarative Course Pack lifecycle, M4 the replay-complete deterministic Learning Kernel, M5 trusted app-owned Node/Python execution, M6 the constrained Pi Provider Hub, M7 the localized Calm Workshop application shell, M8 the closed Activity Frame registry, M9 the AI-Off Adaptive Studio workflow with isolated personal adaptation, M10 the persisted guided Course Designer with reviewable typed Draft proposals, and M11 the per-Course learner-state/session cutover. M12 has a passed local technical preflight, not an accepted or published release candidate.
...
- an Adaptive Studio workflow for manual Draft validation, Preview, immutable Publish/clone, isolated personal adaptation/upstream integration, and optional guided Course Designer proposals that require separate confirmation, Apply, validation, and manual Publish.

The current interview report records completion and answer form. It does **not** establish technical correctness or change mastery. Pre-M3 snapshot bytes and unprovable migration relationships remain preserved; quarantined legacy data does not become valid Course truth. Git-ignored workspace state remains outside test/review freshness, and trusted local execution remains explicitly unsandboxed. M11 freezes legacy writes, retires the hardcoded dashboard, and retains historical read/export and compatibility storage; destructive table removal remains separately gated. Production Course content, complete legal approval, release hardening, and public distribution are not implemented.
...
M1 containment through M11 Course/session cutover are accepted **Implemented baseline** as of 2026-08-10. M6 active role callers use Provider Hub, browser disclosure approval is exact and one-time, cumulative budgets fail closed, and sentinel/adversarial matrices cover the constrained boundary. M7 provides complete UI catalogs and primary navigation; M8 routes Core Alpha activities through the closed renderer registry; M9 keeps upstream and personal revisions separate and creates a new personal Draft for explicit upstream integration. M10 persists the finite Course Designer state machine and reaches `PUBLISHED` only through the separate manual publication gate. M11 makes selected Course, active revision, and current session explicit per Course, preserves simultaneous active sessions across Courses, routes target evidence/review and side effects through exact Course ownership, keeps legacy mutations frozen, and retires the hardcoded dashboard. Production content, licensing/legal approval, and M12 release gates remain open.

M12 local technical preflight passed clean `npm ci`, `npm run verify`, 4/4 E2E, direct and loopback-Compose smoke, active-volume persistence, non-overwriting backup/restore/rollback, disposable real Python execution, dependency policy, SBOM generation, candidate-content inspection, and responsive browser QA. The [row-level release-matrix disposition](docs/product/core-alpha-scope.md#current-gate-evidence) leaves hosted CI and authenticated real-provider smoke open. Professional legal review, approved license/notices/content/trademark artifacts, distribution authorization, and owner sign-off also remain blocking; the repository still grants no license.
...
Before M10, the active database was re-inventoried and backed up without overwrite to `.data/approved-backups/2026-08-10T13-15-59Z-pre-m10-course-designer.sqlite` (whole-file SHA-256 `3486ac30c9b726045d9721612e03600123380f67fa8ffb229ea68e653ef49c72`). Additive migration `0016_course_designer_workflow` reached schema SHA-256 `f23afd4470b6f221273fb15a0f783f08104650cf0fdda728b4f44409e73585aa`; the post-migration inventory reported `integrity_check=ok`, zero foreign-key violations, the exact 17-migration ledger, unchanged session-snapshot inventories, and a verified current-contract no-op replay.

Before M11, the active database was re-inventoried and backed up without overwrite to `.data/approved-backups/2026-08-10T14-56-15Z-pre-m11-course-cutover.sqlite` (whole-file SHA-256 `f76b8ad816094df0b7843864cd48e9e2f955b904753df494e0ef8170536ac710`). Additive migration `0017_learner_course_state` established per-Course state. Verification then exposed that its context trigger also attempted to advance learner state for completed or compatibility-only sessions; additive `0018_learner_course_state_trigger_guard` fixes the source invariant without editing the applied migration. Before `0018`, the exact `0017` database was inventoried and backed up to `.data/approved-backups/2026-08-10T15-22-43Z-pre-m11-trigger-guard.sqlite` (whole-file SHA-256 `0604a330404f94f45e0f6e0faeb62aeb3f0c5b906752825690849b91bdb05f88`). The current schema SHA-256 is `d517a45b89090fba10a6c8db268edf1cef08eb3ad5f67e09f89b00a20be86c40`; postflight reported `integrity_check=ok`, zero foreign-key violations, the exact 19-migration ledger, one selected Course among two per-Course learner-state rows, zero invalid revision/session pointers, zero untracked active sessions, zero target orphans, and 526 retained quarantine rows. Exact replay reported the database already current.
