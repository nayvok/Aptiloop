...
**Implemented baseline.** The principal request path is browser presentation → Hono orchestrator → repositories/pure rules/adapters. The orchestrator composition root (`apps/orchestrator/src/app.ts`) opens, migrates, and seeds SQLite, creates provider adapters, and owns provider sessions and exercise roots.
