...
**Document status:** Approved Core Alpha target, with an explicitly identified Implemented baseline.
...
| Immutable authored revisions and session snapshots | Implemented baseline | Published graph rows are guarded against mutation and sessions capture canonical snapshot JSON and hash (`packages/database/migrations/0001_versioned_curriculum.sql:11-27,151-193`; `packages/database/src/repository.ts:541-701`). Preserve and generalize from curriculum/day/unit names to Course/Revision/Activity without rewriting history. |
...
| Exercise safety primitives | Implemented baseline | Attempts are copied into isolated roots; browser selects only literal command ID `test`; test freshness uses the full diff fingerprint; reviewer mutation is rejected (`apps/orchestrator/src/app.ts:684-764,779-868,870-953`). Preserve these controls behind the generic Execution Fabric contract; do not expose its executable plan to Course Packs or models. |
...
1. **Implemented baseline.** Legacy `/api/dashboard` reads the hardcoded `weekOneCurriculum`, and legacy session completion accepts the old route and injects fixed mastery, mistakes, and cards (`apps/orchestrator/src/app.ts:299-367,374-453`). This bypasses versioned evidence and the deterministic kernel.
2. **Implemented baseline.** The versioned path selects one recently updated active curriculum with `LIMIT 1`, while `learner_state` and the active-session unique index encode one global current session (`apps/orchestrator/src/learning-v2.ts:187-210`; `packages/database/migrations/0001_versioned_curriculum.sql:132-149`). This is not yet a Course library or per-Course learner branch.
...
4. **Implemented baseline.** Browser input can override the configured provider and model for `/api/agent/stream` (`apps/orchestrator/src/app.ts:455-466`). Selection and capability policy are therefore not yet centralized.
5. **Implemented baseline.** Non-review Codex sessions receive `workspace-write`, and non-review OpenCode sessions do not receive the reviewer's deny-all tool policy (`packages/codex-provider/src/codex-provider.ts:108-110,183-190`; `packages/opencode-provider/src/provider.ts:468-470,577-579`). The target must not expose arbitrary provider tools for any role.
6. **Implemented baseline.** Mock is the persisted default for role selections (`apps/orchestrator/src/app.ts:1560-1577`). Mock is a test/development facility, not a production or user-facing fallback.
